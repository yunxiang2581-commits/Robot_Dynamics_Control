# 学习副本说明:
# - 原始文件来自 external/mink_upstream/src/mink/limits/configuration_limit.py。
# - ConfigurationLimit 根据 joint range 限制积分后的 q 不越界。
# - 阅读重点: position limit 如何转换成当前这一步允许的 delta_q。
...
class ConfigurationLimit(Limit):
    # 学习提示: 这个 limit 和 VelocityLimit 不同，它关心的是下一步 q 是否仍在 joint range 内。
    ...
    def compute_qp_inequalities(
        ...
        # 学习提示: 这里根据当前 q 到上下限的剩余距离，构造对 delta_q 的不等式。
