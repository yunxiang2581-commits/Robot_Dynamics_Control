# 学习副本说明:
# - 原始文件来自 external/mink_upstream/src/mink/tasks/frame_task.py。
# - FrameTask 把末端 frame/site 的目标位姿转换成 6D error 和 task Jacobian。
# - 阅读重点: position/orientation error 如何和 Jacobian 一起形成 QP objective。
...
class FrameTask(Task):
    # 学习提示: 这是 UR5e 示例中末端执行器跟踪目标 pose 的核心 task。
    ...
    def compute_error(self, configuration: Configuration) -> np.ndarray:
        # 学习提示: 误差 e 是 task 希望减少的量，通常包含位置和姿态两部分。
    ...
    def compute_jacobian(self, configuration: Configuration) -> np.ndarray:
        # 学习提示: Jacobian 把关节速度映射到 frame 的空间速度，是 IK 线性化的核心。
    ...
    def compute_qp_objective(self, configuration: Configuration) -> Objective:
        # 学习提示: task 最终要变成 QP 的 H、c，供 solve_ik 累加。
