# 学习副本说明:
# - 原始文件来自 external/mink_upstream/src/mink/tasks/posture_task.py。
# - PostureTask 让当前 q 保持接近参考 q，常作为正则项或弱约束。
# - 阅读重点: posture error 为什么在 velocity/tangent space 中表达。
...
class PostureTask(Task):
    # 学习提示: 这个 task 不直接控制末端，而是避免姿态漂移或奇怪折叠。
    ...
    def compute_error(self, configuration: Configuration) -> np.ndarray:
        # 学习提示: q 和 target_q 的差不能总是简单相减，尤其遇到 quaternion / floating-base。
    ...
    def compute_jacobian(self, configuration: Configuration) -> np.ndarray:
        # 学习提示: posture task 对多数 actuated dof 接近 identity Jacobian。
