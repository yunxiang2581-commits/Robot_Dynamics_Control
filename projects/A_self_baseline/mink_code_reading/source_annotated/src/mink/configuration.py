# 学习副本说明:
# - 原始文件来自 external/mink_upstream/src/mink/configuration.py。
# - 本文件是 mink 的状态中心: 管理 MuJoCo model/data/q，并提供 pose、Jacobian、积分接口。
# - 阅读重点: 为什么 task 和 solver 都围绕 Configuration 取当前状态。
...
class Configuration:
    # 学习提示: 这个类相当于 A 项目里 A02/A03/A04 中状态更新、pose 查询、Jacobian 查询的集合。
    ...
    * Integrating velocities to update configurations.
    ...
    def update(self, q: np.ndarray | None = None) -> None:
        # 学习提示: update 的核心是让 MuJoCo data 和当前 q 保持一致。
        # 后续 pose/Jacobian 查询都依赖这个刷新后的 data。
    ...
    def integrate(self, velocity: np.ndarray, dt: float) -> np.ndarray:
        # 学习提示: 用 MuJoCo 的积分函数生成新 q，比直接 q += v * dt 更适合 quaternion / floating-base。
    ...
    def integrate_inplace(self, velocity: np.ndarray, dt: float) -> None:
        # 学习提示: inplace 版本会把积分结果写回当前 configuration，并重新 update。
