...
RUN_ROOT="$RUNS_ROOT/${TS}_walk_wbc_t4_inplace_visual_wslg"
CONTAINER_NAME="projectc_walk_wbc_t4_visual_$TS"

mkdir -p "$RUN_ROOT/commands" "$RUN_ROOT/logs" "$RUN_ROOT/runtime_record"

cat > "$RUN_ROOT/run_info.txt" <<EOF
mode: foreground visual smoke
demo: walk_wbc T4 in-place stepping + right-hand Cartesian hold
image: $IMAGE_NAME
build_root: $BUILD_ROOT
run_root: $RUN_ROOT
container_name: $CONTAINER_NAME
env: OPENLOONG_T4_IN_PLACE=1 OPENLOONG_T4_CART_HAND=1
