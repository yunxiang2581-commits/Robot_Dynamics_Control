...
SMOKE_SECONDS="${1:-30}"
T1_PERTURB_X="${OPENLOONG_T1_PERTURB_X:-}"
T1_VIS_FORCE="${OPENLOONG_T1_VIS_FORCE:-}"
T1_FORCE_N="${OPENLOONG_T1_FORCE_N:-}"
TS=$(date +%Y%m%d_%H%M%S)
RUN_ROOT="$RUNS_ROOT/${TS}_walk_wbc_t1_visual_wslg"
CONTAINER_NAME="projectc_walk_wbc_t1_visual_$TS"

mkdir -p "$RUN_ROOT/commands" "$RUN_ROOT/logs" "$RUN_ROOT/runtime_record"

cat > "$RUN_ROOT/run_info.txt" <<EOF
mode: foreground visual smoke
demo: walk_wbc T1 stand-only visual
image: $IMAGE_NAME
build_root: $BUILD_ROOT
run_root: $RUN_ROOT
container_name: $CONTAINER_NAME
env: OPENLOONG_T1_STAND_ONLY=1 OPENLOONG_T1_CART_HAND=1${T1_PERTURB_X:+ OPENLOONG_T1_PERTURB_X=$T1_PERTURB_X}${T1_VIS_FORCE:+ OPENLOONG_T1_VIS_FORCE=$T1_VIS_FORCE}${T1_FORCE_N:+ OPENLOONG_T1_FORCE_N=$T1_FORCE_N}
...
docker_args+=(
    ...
    -e OPENLOONG_T1_STAND_ONLY=1
    -e OPENLOONG_T1_CART_HAND=1
    -v /tmp/.X11-unix:/tmp/.X11-unix:rw
)

if [[ -n "$T1_PERTURB_X" ]]; then
    docker_args+=(-e "OPENLOONG_T1_PERTURB_X=$T1_PERTURB_X")
fi
if [[ -n "$T1_VIS_FORCE" ]]; then
    docker_args+=(-e "OPENLOONG_T1_VIS_FORCE=$T1_VIS_FORCE")
fi
if [[ -n "$T1_FORCE_N" ]]; then
    docker_args+=(-e "OPENLOONG_T1_FORCE_N=$T1_FORCE_N")
fi
