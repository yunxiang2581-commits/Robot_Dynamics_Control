...
RUN_ROOT="$RUNS_ROOT/${TS}_walk_wbc_t4_inplace_baseline_wslg"
CONTAINER_NAME="projectc_walk_wbc_t4_baseline_$TS"

mkdir -p "$RUN_ROOT/commands" "$RUN_ROOT/logs" "$RUN_ROOT/runtime_record"

cat > "$RUN_ROOT/run_info.txt" <<EOF
mode: foreground visual smoke
demo: walk_wbc T4 in-place stepping + right-hand baseline arm lock
image: $IMAGE_NAME
build_root: $BUILD_ROOT
run_root: $RUN_ROOT
container_name: $CONTAINER_NAME
env: OPENLOONG_T4_IN_PLACE=1 OPENLOONG_T4_BASELINE=1
