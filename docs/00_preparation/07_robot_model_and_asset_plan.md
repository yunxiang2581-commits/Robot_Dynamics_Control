...
| --- | --- | --- | --- | --- |
| URDF | A 项 Pinocchio 模型加载 | `assets/urdf/` 或外部路径 | 小型示例可提交 | 大型 mesh 不随 URDF 一起提交 |
| MJCF | MuJoCo 仿真和 RL | `assets/mjcf/` 或外部路径 | 小型示例可提交 | 复杂模型优先引用官方仓库 |
| mesh | 视觉和碰撞模型 | 外部数据目录 | 默认不提交 | 大文件通过下载说明或 submodule 管理 |
| 图片 | 结果图 | `outputs/figures/` | 可提交精选图 | 只提交小体积展示图 |
| 视频 | 控制或 Play demo | `outputs/videos/` | 默认不提交 | 可在 release 或网盘外链管理 |
| 日志 | 安装、编译、训练记录 | `outputs/logs/` | 默认不提交完整日志 | 文档中保留摘要 |
| 权重 | RL policy、checkpoint | `outputs/checkpoints/` | 不提交 | 只记录来源和指标 |
| 报告 | 实验总结 | `outputs/reports/` | 可提交 | 使用 Markdown 或小型 PDF |
...
| `external/*/src/` | 忽略第三方源码 | 防止把大仓库复制进来 |
