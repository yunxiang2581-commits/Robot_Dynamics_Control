# Step 13A: A05 Task + Limit + QP-IK TODO Skeleton

## 1. 为什么 A05 先做 TODO skeleton
...
因此 Step 13A 只做学习型 TODO skeleton：先明确每个模块要实现什么、为什么存在、对标 mink 的哪个概念、推荐 API、输入输出和验证方式。

## 2. A05 与 A04 的关系

A04 已完成 position-mode DLS differential IK，证明 `attachment_site` 可以通过 Jacobian 驱动位置误差收敛。A05 继续复用 A01-A04 的模型、site、Jacobian 验证和 A04 q trajectory，把无约束 DLS 扩展成受约束 QP-IK 的设计骨架。

A04 解决“能不能用 Jacobian 收敛”；A05 规划“如何在 task weights 和 limits 下安全地求 `dq`”。
...
Step 13A 不在 Python 中实现真实 QP-IK。核心逻辑继续保留 `NotImplementedError`，避免误以为 A05 已经可以生成受约束轨迹。
...
Step 13B：A05 minimal box-constrained QP-IK。

建议第一版只实现 position-only FrameTask + VelocityLimit，并验证：
